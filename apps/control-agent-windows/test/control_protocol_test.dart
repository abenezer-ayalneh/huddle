...
void main() {
  ...
  test('accepts a bounded packet and rejects replay-safe malformed variants',
    ...
    expect(
        ControlProtocol.decode(
            List<int>.filled(maximumControlPacketBytes + 1, 0)),
        isNull);
  });
  ...
  test('bounds clipboard updates before data publication', () {
    ...
    expect(
        ControlProtocol.isTransferableClipboardText(
            'x' * (maximumClipboardTextBytes + 1)),
        isFalse);
    expect(
      () => ControlProtocol.clipboardUpdate(
          sessionId: 'session',
          revision: 1,
          text: 'x' * (maximumClipboardTextBytes + 1)),
      throwsArgumentError,
    );
  });
}
