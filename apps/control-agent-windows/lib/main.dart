...
class _ControlAgentAppState extends State<ControlAgentApp> {
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(
            seedColor: const Color(0xffd6a8ff),
            brightness: Brightness.dark,
            surface: const Color(0xff100d16)),
                    ...
                    children: [
                      Text('HUDDLE',
                          style: Theme.of(context)
                              ...
                              ?.copyWith(
                                  letterSpacing: 3,
                                  color: const Color(0xffd6a8ff))),
                      const SizedBox(height: 10),
                      Text(_title(phase),
                          style: Theme.of(context).textTheme.headlineSmall),
                      const SizedBox(height: 10),
                      Text(_description(phase),
                          style: Theme.of(context)
                              .textTheme
                              .bodyLarge
                              ?.copyWith(color: const Color(0xffd4cddd))),
                      const SizedBox(height: 24),
                      if (phase == AgentPhase.waitingForLink ||
                        ...
                        TextField(
                          ...
                          decoration: const InputDecoration(
                              labelText: 'Huddle Control Agent link',
                        ...
                        Wrap(
                          ...
                          children: [
                            FilledButton(
                                onPressed: () => _agent
                                    .acceptLink(_linkController.text.trim()),
                                child: const Text('Open approved link')),
                            OutlinedButton(
                                onPressed: _elevateManualLink,
                                child: const Text(
                                    'Open in administrator-app mode')),
                          ],
                        ),
                      ],
                      if (phase == AgentPhase.trustRequired) ...[
                        Text(
                            'Trust ${_agent.apiOrigin} on this computer before this agent redeems the one-time link. This stores only the server origin, never the room or token.'),
                        const SizedBox(height: 12),
                        FilledButton(
                            onPressed: _agent.trustServer,
                            child: const Text('Trust this Huddle server')),
                      ],
                      if (phase == AgentPhase.readyToConnect)
                        const Padding(
                            padding: EdgeInsets.symmetric(vertical: 12),
                            child: LinearProgressIndicator()),
                      if (phase == AgentPhase.connecting ||
                          phase == AgentPhase.awaitingActivation)
                        const Padding(
                            padding: EdgeInsets.symmetric(vertical: 12),
                            child: LinearProgressIndicator()),
                      if (phase == AgentPhase.chooseDisplay ||
                          phase == AgentPhase.readyToStart) ...[
                        Text(_agent.displayName == null
                            ? 'Select one entire physical display. Huddle shares no audio and does not offer window sharing.'
                            : 'Selected display: ${_agent.displayName}'),
                        const SizedBox(height: 12),
                        Wrap(
                          spacing: 10,
                          runSpacing: 10,
                          children: [
                            OutlinedButton(
                                ...
                                child: Text(_agent.displayName == null
                                    ? 'Select display'
                                    : 'Change display')),
                            if (phase == AgentPhase.readyToStart)
                              FilledButton(
                                  onPressed: _agent.start,
                                  child: const Text('Start Remote Control')),
                          ],
                        ),
                      ],
                      if (phase == AgentPhase.active ||
                          phase == AgentPhase.switchingDisplay) ...[
                        Text(_agent.elevated
                        ...
                        Wrap(
                          ...
                          children: [
                            OutlinedButton(
                                onPressed: phase == AgentPhase.active
                                    ? () => _agent.switchDisplay(context)
                                    : null,
                                child: const Text('Change display')),
                            FilledButton(
                                onPressed: _agent.stop,
                                style: FilledButton.styleFrom(
                                    backgroundColor: const Color(0xffc54d61)),
  ...
  String _title(AgentPhase phase) => switch (phase) {
        ...
        AgentPhase.readyToConnect => 'Preparing the approved room',
  ...
  String _description(AgentPhase phase) => switch (phase) {
        AgentPhase.waitingForLink =>
          'The Sharer starts inside an active Huddle room. Paste the full one-time link if the browser could not open this app.',
        AgentPhase.trustRequired =>
          'The link names a Huddle server this computer has not trusted yet.',
        AgentPhase.readyToConnect =>
          'The approved link is being connected. The Sharer still chooses a display and explicitly starts Remote Control locally.',
        ...
        AgentPhase.readyToStart =>
          'Choose the display locally, then explicitly start the attended session.',
        AgentPhase.active ||
        AgentPhase.switchingDisplay =>
          'Input and clipboard sharing stop immediately if the grant, room, display, or local Windows session ends.',
        AgentPhase.ended =>
          'The agent disconnected and released all held input.',
        AgentPhase.failed =>
          'Correct the link or use the browser room to issue a fresh approved link.',
      };
}
