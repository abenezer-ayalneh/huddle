...
constexpr const wchar_t kWindowClassName[] = L"HUDDLE_CONTROL_AGENT_WIN32_WINDOW";
constexpr const wchar_t kBrightnessKey[] =
    L"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize";
constexpr const wchar_t kBrightnessValue[] = L"AppsUseLightTheme";
...
void EnableFullDpiSupportIfAvailable(HWND hwnd) {
  ...
  const auto enable_non_client_dpi_scaling = reinterpret_cast<EnableNonClientDpiScaling*>(
      GetProcAddress(user32_module, "EnableNonClientDpiScaling"));
  if (enable_non_client_dpi_scaling) enable_non_client_dpi_scaling(hwnd);
...
bool Win32Window::Create(const std::wstring& title, const Point& origin, const Size& size) {
  ...
  HWND window = CreateWindow(WindowClassRegistrar::GetInstance()->GetWindowClass(), title.c_str(),
      WS_OVERLAPPEDWINDOW, Scale(origin.x, scale_factor), Scale(origin.y, scale_factor),
      Scale(size.width, scale_factor), Scale(size.height, scale_factor), nullptr, nullptr,
...
void Win32Window::UpdateTheme(HWND const window) {
  ...
  const LSTATUS result = RegGetValue(HKEY_CURRENT_USER, kBrightnessKey, kBrightnessValue,
