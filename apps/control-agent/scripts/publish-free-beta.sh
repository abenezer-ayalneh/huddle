...
# Publish only the output made by build-free-beta.sh.
...
if gh release view "$TAG" --repo "$REPOSITORY" >/dev/null 2>&1; then
  ...
  gh release edit "$TAG" --repo "$REPOSITORY" --title 'Huddle Control Agent · Apple Silicon no-cost beta' \
    --notes 'Unsigned and unnotarized Apple-Silicon beta. Verify the attached SHA-256 checksum before opening the DMG, then use macOS Privacy & Security → Open Anyway.'
else
