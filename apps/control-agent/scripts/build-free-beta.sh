...
# Build the deliberately ad-hoc, zero-cost Apple-Silicon beta. Sparkle's
...
# This lane intentionally uses a fresh ad-hoc application signature for every
# published build. macOS therefore treats each installation or update as a new
# Screen Recording and Accessibility client; testers must grant those
# permissions to the exact release candidate.
...
CODE_SIGN_IDENTITY=- ARCHITECTURE=arm64 AGENT_VERSION="$VERSION" AGENT_BUILD_VERSION="$BUILD_VERSION" \
...
echo "Built ad-hoc signed, unnotarized Apple-Silicon beta: $DMG"
