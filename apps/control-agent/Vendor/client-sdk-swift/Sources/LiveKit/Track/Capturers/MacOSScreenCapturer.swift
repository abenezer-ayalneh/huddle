...
public class MacOSScreenCapturer: VideoCapturer, @unchecked Sendable {
    ...
    enum DisplayFilterKind: Equatable {
        case entireDisplay
        case applications
    }

    static func displayFilterKind(for options: ScreenShareCaptureOptions) -> DisplayFilterKind {
        options.captureEntireDisplay ? .entireDisplay : .applications
    }

    static func captureDimensions(displayWidth: Int, displayHeight: Int) -> Dimensions {
        // Keep the existing high-density capture scale, but derive it from the
        // selected SCDisplay instead of CGMainDisplayID().
        Dimensions(width: Int32(max(displayWidth, 1) * 2), height: Int32(max(displayHeight, 1) * 2))
    }
    ...
    override public func startCapture() async throws -> Bool {
        ...
        var captureWidth = CGDisplayPixelsWide(CGMainDisplayID()) * 2
        var captureHeight = CGDisplayPixelsHigh(CGMainDisplayID()) * 2
        ...
        {
            if Self.displayFilterKind(for: options) == .entireDisplay {
                // Do not exclude the Control Agent or any other window. This is
                // the literal physical display selected by the Sharer.
                filter = SCContentFilter(display: nativeDisplay, excludingWindows: [])
            } else {
            ...
            let dimensions = Self.captureDimensions(displayWidth: displaySource.width, displayHeight: displaySource.height)
        ...
        // Try to capture in max resolution. For a selected display the values
        // above came from that SCDisplay, never from the primary monitor.
        #if compiler(>=6.4)
        LKObjCHelpers.setWidth(captureWidth,
