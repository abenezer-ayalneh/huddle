...
export default function RemoteControlToast({
  ...
  if (incoming) {
    return (
      <div
        role="dialog"
        aria-modal="true"
        className="pointer-events-auto flex max-w-[min(94vw,34rem)] flex-col gap-3 rounded-2xl bg-black/80 p-4 text-white ring-1 ring-magenta/50 backdrop-blur"
      >
        ...
        <div className="flex justify-end gap-2">
          <button type="button" onClick={onDeny} className="rounded-lg bg-white/10 px-3 py-1.5 text-xs text-white/75 hover:bg-white/20">
            <X className="mr-1 inline h-3.5 w-3.5" />
            Deny
          </button>
          <button type="button" onClick={onApprove} className="rounded-lg bg-cyan/20 px-3 py-1.5 text-xs text-cyan ring-1 ring-cyan/45 hover:bg-cyan/30">
  ...
  return (
    <button
      ...
      role="status"
      onClick={onDismiss}
      className={`pointer-events-auto rounded-full bg-black/75 px-4 py-2 text-sm ring-1 ${notice.tone === 'error' ? 'text-amber-200 ring-amber-300/35' : 'text-white/85 ring-cyan/35'}`}
    >
      {notice.message}
    </button>
  );
